...
- 連線測試指令、互動式登入指令、`~/.ssh/config` Host 區塊產生器（可新增/移除多組、複製或匯出成文字檔）。
- known_hosts 主機金鑰維護：`ssh-keygen -R` 清除主機重建後失效的舊金鑰記錄，再以 `ssh-keyscan` 加入目前金鑰。
- rsync 進階旗標：`--dry-run` 預覽、`--partial` 斷點續傳、`--delete`（並在未搭配預覽模式時顯示風險提醒）、`--progress`、`--exclude`（逗號分隔多組樣式）。
- 具名連線設定檔：儲存、切換、刪除多組連線設定，可匯出/匯入 JSON 檔案。
- 產生的指令可一鍵匯出成可執行腳本檔（`.sh` / `.ps1`），不只是複製貼上。
